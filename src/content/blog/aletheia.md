...
Most AI decompilers I've looked at train a model to go from binary to C. The model learns what C looks like and generates plausible code that compiles to something different from the input. The BLEU scores look great in the paper. The output tends to be wrong in ways that matter.
...
MLIR (Multi-Level Intermediate Representation) is a compiler framework with a dialect system that separates concerns into composable layers. Three dialects form the target representation:
...
Ghidra's p-code is a production-grade, architecture-independent IL. Every supported architecture (x86, ARM, MIPS, PowerPC, SPARC, AVR, Hexagon, dozens more) gets mechanically translated to the same p-code via SLEIGH specs. The vocabulary is small and regular, which should make it a reasonable tokenizer target.
...
For training the MLIR output side specifically, [Polygeist](https://github.com/llvm/Polygeist) (cgeist) compiles C directly to SCF-level MLIR without going through LLVM IR:
